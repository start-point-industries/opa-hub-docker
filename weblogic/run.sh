...
if ! docker run --name opa-weblogic-full -e ADMIN_PASSWORD=Passw0rd -p 7001:7001 --link opa-mysql-full:opa-mysql-full --health-cmd="curl -f http://localhost:7001/console || exit 1" -d container-registry.oracle.com/middleware/weblogic:12.2.1.3; then
    exit 1
fi
